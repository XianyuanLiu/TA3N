...
# parser.add_argument('--data_path', type=str, required=False, default='F:/Datasets/EgoAction/EPIC/EPIC_KITCHENS_2018/frames_rgb_flow/rgb/train/', help='source path')
parser.add_argument('--data_path', type=str, required=False, default='F:/Datasets/Test_fe/', help='source path')
parser.add_argument('--video_in', type=str, required=False, default='P01', help='name of input video dataset')
parser.add_argument('--feature_in', type=str, required=False, default='RGB-feature', help='name of output frame dataset')
parser.add_argument('--input_type', type=str, required=False, default='frames', choices=['video', 'frames'], help='input types for videos')
parser.add_argument('--structure', type=str, required=False, default='tsn', choices=['tsn', 'imagenet'], help='data structure of output frames')
...
parser.add_argument('--class_file', type=str, required=False, default='class_list.txt', help='process the classes only in the class_file')
...
# root folder
path_output = args.data_path + args.feature_in + '_' + args.base_model + '/' + args.video_in + '/'
if args.structure != 'tsn':
	path_output = args.data_path + args.feature_in + '-' + args.structure + '/' + args.video_in + '/'
...
def extract_features(video_file):
	...
	num_exist_files = len(os.listdir(path_output + class_name + '/'))
